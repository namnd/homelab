resource "kubernetes_namespace_v1" "media" {
  metadata {
    name = "media"
  }
}

resource "kubernetes_persistent_volume_claim_v1" "media_data" {
  metadata {
    name      = "media-data"
    namespace = kubernetes_namespace_v1.media.id
  }

  spec {
    access_modes = ["ReadWriteMany"]
    resources {
      requests = {
        storage = "80G"
      }
    }

    storage_class_name = "longhorn"
  }
}

resource "kubernetes_config_map_v1" "youtube_dl_config" {
  metadata {
    name      = "youtube-dl-config"
    namespace = kubernetes_namespace_v1.media.id
  }

  data = {
    "args.conf"    = file("${path.module}/youtube-dl-config/args.conf")
    "channels.txt" = file("${path.module}/youtube-dl-config/channels.txt")
  }
}

resource "helm_release" "youtube_dl" {
  name       = "youtube-dl"
  repository = "https://namnd.github.io/helm-charts"
  chart      = "youtube-dl"
  version    = "0.2.0"

  create_namespace = false
  namespace        = kubernetes_namespace_v1.media.id

  set = [
    {
      name  = "image.tag"
      value = "v2026.06.09"
    },
    {
      name  = "env[0].name"
      value = "youtubedl_webui"
    },
    {
      name  = "env[0].value"
      value = "true"
    },
    {
      name  = "persistence.enabled"
      value = true
    },
    {
      name  = "persistence.downloads.existingClaim"
      value = kubernetes_persistent_volume_claim_v1.media_data.metadata[0].name
    },
    {
      name  = "config.enabled"
      value = true
    },
    {
      name  = "config.configMap"
      value = kubernetes_config_map_v1.youtube_dl_config.metadata[0].name
    },
  ]
}

resource "kubernetes_ingress_v1" "youtube_dl_ingress" {
  metadata {
    name      = "youtube-dl"
    namespace = kubernetes_namespace_v1.media.id
  }

  spec {
    ingress_class_name = "nginx"
    rule {
      host = "${local.youtube_dl_subdomain}.${local.domain}"
      http {
        path {
          path      = "/"
          path_type = "Prefix"
          backend {
            service {
              name = "youtube-dl"
              port {
                number = 8080
              }
            }
          }
        }
      }
    }
  }
}

resource "helm_release" "jellyfin" {
  name       = "jellyfin"
  repository = "https://jellyfin.github.io/jellyfin-helm"
  chart      = "jellyfin"
  version    = "2.6.0"

  create_namespace = false
  namespace        = kubernetes_namespace_v1.media.id

  set = [
    {
      name  = "persistence.media.existingClaim"
      value = kubernetes_persistent_volume_claim_v1.media_data.metadata[0].name
    }
  ]
}

resource "kubernetes_ingress_v1" "jellyfin_ingress" {
  metadata {
    name      = "jellyfin"
    namespace = kubernetes_namespace_v1.media.id
  }

  spec {
    ingress_class_name = "nginx"
    rule {
      host = "video.namnd.com"
      http {
        path {
          path      = "/"
          path_type = "Prefix"
          backend {
            service {
              name = "jellyfin"
              port {
                number = 8096
              }
            }
          }
        }
      }
    }
  }
}
